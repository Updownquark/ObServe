...
<quick xmlns:base="Quick-Base v0.1" xmlns:config="Expresso-Config v0.1" with-extension="window" title="app.qwysiwyg.title" close-action="exit"
	...
	<head>
		...
		<models>
			...
			<model name="app">
				<constant name="qwysiwyg">new Qwysiwyg()</constant>
		...
		<style-sheet>
			<import-style-sheet name="qwysiwygStyle" ref="qwysiwyg.qss" />
		</style-sheet>
	</head>
	<tabs>
		<split tab-id="`documents`" tab-name="`Documents`" orientation="vertical" split-position="config.split * `1%`">
			<tabs>
				<scroll tab-id="`main`" tab-name="app.qwysiwyg.getDocumentName()">
					...
					<text-area role="content" editable="false" tooltip="app.qwysiwyg.document.getTooltip()">
						...
						<dynamic-styled-document root="app.qwysiwyg.document.getRoot()" children="node.children" post-text="node.getPostText()"
							...
							<text-style>
								<style style-set="qwysiwygStyle.documentStyle" />
							</text-style>
		...
		<tabs tab-id="`toolkits`" tab-name="`Toolkits`" selected="app.qwysiwyg.selectedToolkit">
			<tab-set values="app.qwysiwyg.toolkits">
				<text-area tab-name="tabId.toolkit.toString()" editable="false" tooltip="hoveredNode==null ? null : hoveredNode.getTooltip()">
					<model>
						<value name="hoveredNode" type="StyledQonfigToolkit.Node" />
					</model>
					<dynamic-styled-document root="tabId==null ? null : tabId.getRoot()" children="node.getChildren()"
						selection-start-value="tabId.selected">
						<model>
							<hook name="updateHoverNode" on="node">hoveredNode=node</hook>
						</model>
						<text-style>
							<style attr="mouse-cursor" if="node!=null &amp;&amp; node.getLinkTarget()!=null">HAND</style>
							<style style-set="qwysiwygStyle.toolkitStyle" />
						</text-style>
					</dynamic-styled-document>
					<on-click>
						<event-filter>hoveredNode!=null &amp;&amp; hoveredNode.getLinkTarget()!=null</event-filter>
						app.qwysiwyg.goToToolkitReference(hoveredNode.getLinkTarget())
					</on-click>
				</text-area>
			</tab-set>
		</tabs>
