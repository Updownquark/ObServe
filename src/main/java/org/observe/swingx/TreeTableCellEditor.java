...
public class TreeTableCellEditor implements TableCellEditor {
	...
	private final TreeTableEditorHolder theEditorHolder;
	private final Supplier<TableCellEditor> theTreeTableEditor;

	public TreeTableCellEditor(JTree tree, Supplier<TableCellEditor> fromTreeTable) {
		if (tree == null) {
			throw new IllegalArgumentException("null tree");
		}
		theEditorHolder = new TreeTableEditorHolder();
		theTreeTableEditor = fromTreeTable;
		// JW: no need to...
	...
	public Component getTableCellEditorComponent(JTable table, Object value,
		...
		Component component = theTreeTableEditor.get().getTableCellEditorComponent(table, value,
			isSelected, row, column);
		// JW: this implementation is not bidi-compliant, need to do better
		initEditorOffset(table, row, column, isSelected, component);
		return theEditorHolder;
	}
	 ...
	 * @param isSelected
	 */
	protected void initEditorOffset(JTable table, int row, int column, boolean isSelected, Component editorComponent) {
		if (tree == null)
			return;
		...
		Component treeRenderer = tcr.getTreeCellRendererComponent(tree, node,
			isSelected, expanded, leaf, row, false);

		theEditorHolder.init(row, column, table, tree, treeRenderer, editorComponent);
	}

	@Override
	public boolean isCellEditable(EventObject e) {
		TableCellEditor treeTableEditor = theTreeTableEditor.get();
		return treeTableEditor != null && treeTableEditor.isCellEditable(e);
	}

	@Override
	public Object getCellEditorValue() {
		TableCellEditor treeTableEditor = theTreeTableEditor.get();
		return treeTableEditor == null ? null : treeTableEditor.getCellEditorValue();
	}

	@Override
	public boolean shouldSelectCell(EventObject anEvent) {
		TableCellEditor treeTableEditor = theTreeTableEditor.get();
		return treeTableEditor != null && treeTableEditor.shouldSelectCell(anEvent);
	}

	@Override
	public boolean stopCellEditing() {
		TableCellEditor treeTableEditor = theTreeTableEditor.get();
		return treeTableEditor != null && treeTableEditor.stopCellEditing();
	}
	...
	public void cancelCellEditing() {
		...
		if (treeTableEditor != null)
			treeTableEditor.cancelCellEditing();
	}
	...
	public void addCellEditorListener(CellEditorListener l) {
		TableCellEditor treeTableEditor = theTreeTableEditor.get();
		if (treeTableEditor != null)
			treeTableEditor.addCellEditorListener(l);
	}
	...
	public void removeCellEditorListener(CellEditorListener l) {
		TableCellEditor treeTableEditor = theTreeTableEditor.get();
		if (treeTableEditor != null)
			treeTableEditor.removeCellEditorListener(l);
	}

	/**
	 * Component used by TreeTableCellEditor. The only thing this does is to override the <code>reshape</code> method, and to ALWAYS make
	 * the x location be <code>offset</code>.
	 */
	static class TreeTableEditorHolder extends JPanel {
		private int iconWidth;

		TreeTableEditorHolder() {
			super(new JustifiedBoxLayout(false).mainJustified().crossJustified());
		}

		void init(int row, int column, JTable table, JTree tree, Component treeRenderer, Component editorComponent) {
			this.column = column;
			this.row = row;
			this.table = table;
			this.tree = tree;
			updateIconWidth(treeRenderer);
		 ...
		 * @param treeComponent
		 */
		private void updateIconWidth(Component treeRenderer) {
			iconWidth = 0;
			if (!(treeRenderer instanceof JLabel))
				return;
			Icon icon = ((JLabel) treeRenderer).getIcon();
			if (icon != null) {
				iconWidth = icon.getIconWidth() + ((JLabel) treeRenderer).getIconTextGap();
			}
