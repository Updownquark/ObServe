 ...
 */
public interface ObservableMultiMapEvent<K, V> extends ObservableMapEvent<K, V> {
	 ...
	 * @param <V> The value-type of the map
	 */
	public class Default<K, V> extends ObservableMapEvent.Default<K, V> implements ObservableMultiMapEvent<K, V> {
		 ...
		 *        key value that does not affect the contents of the key's values. In this case, {@link #getIndex()} will be -1 and
		 ...
		 */
		public Default(ElementId keyElementId, ElementId valueElementId, int keyIndex, int valueIndex, CollectionChangeType type, K oldKey,
			K key, V oldValue, V newValue, Object... causes) {
